> **Vendored copy** — this directory preserves the upstream README, LICENSE (Apache-2.0), and plugin manifest of the
> [pr-review-toolkit](https://github.com/anthropics/claude-plugins-official/tree/main/plugins/pr-review-toolkit) plugin.
> The agents live in `.claude/agents/` and the `/review-pr` command in `.claude/commands/` so they load in any
> Claude Code or Claude Cowork session on this repository. The upstream `/pr-review-toolkit:review-pr` command is `/review-pr` here.
